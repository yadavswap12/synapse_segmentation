...
def makeInputData2(tile, ss_bb, resize, img_num, tile_id, input_data_directory, sema):
...
def makeOutputData2(tile, ss_bb, resize, img_num, tile_id, output_data_directory, sema):
...
def makeInputData(tile, ss_bb_list, resize, img_num, tile_id, input_data_directory):
    ...
    for ss_bb in ss_bb_list:
        ...
        process = multiprocessing.Process(target = makeInputData2, args=(tile, ss_bb, resize, img_num, tile_id, input_data_directory, sema))                
        jobs.append(process)
        process.start()
...
def makeOutputData(ss_bb_list, bb_list, img_num, tile_id, output_data_directory):
    ...
    for ss_bb in ss_bb_list:
        ...
        # Assign process for each tile.
        # process = multiprocessing.Process(target = locsPerTile, args=(h5_file, tile_start_pix_y, tile_start_pix_x, tile_size, max_locs_per_pixel, nm_per_pixel, storm_image_scale, locs_storm_file_name, locs_tile_file_name, locs_storm))
        process = multiprocessing.Process(target = makeOutputData2, args=(ss_bb, bb_list, img_num, tile_id, output_data_directory, sema))                
        jobs.append(process)
        process.start()
...
# def makeData(tile, ss_bb_list, bb_list, resize, img_num, tile_id, input_data_directory, output_data_directory):
...
if __name__ == "__main__":
    ...
    if not os.path.exists(storm_exp_directory + "input_data_tiles_size_{}_copy\\" .format(tile_size)):
        os.mkdir(storm_exp_directory + "input_data_tiles_size_{}_copy\\" .format(tile_size))

    input_data_directory = storm_exp_directory + "input_data_tiles_size_{}_copy\\" .format(tile_size)
    ...
    # Make directory to store image tiles.
    if not os.path.exists(storm_exp_directory + "output_data_tiles_size_{}_copy\\" .format(tile_size)):
        os.mkdir(storm_exp_directory + "output_data_tiles_size_{}_copy\\" .format(tile_size))

    output_data_directory = storm_exp_directory + "output_data_tiles_size_{}_copy\\" .format(tile_size)
    ...
    tiles_df = tiles_df.sample(n=100, random_state=123)    
    ...
    for img_num in tiles_df["Num_image"].unique():
        ...
        for idx in img_df.index:
            ...
            # makeData(tile, ss_bb_list, bb_list, resize, img_num, tile_id, input_data_directory, output_data_directory)
            makeInputData(tile, ss_bb_list, resize, img_num, tile_id, input_data_directory)
            makeOutputData(ss_bb_list, bb_list, img_num, tile_id, output_data_directory)                 
